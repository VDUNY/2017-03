...
namespace DemoActor
{
	...
	internal class DemoActor : Actor, IDemoActor
	{
		...
		protected override Task OnActivateAsync()
		{
			...
			// The StateManager is this actor's private state store. Data stored in the StateManager
			// will be replicated for high-availability for actors that use volatile or persisted state
			// storage. Any serializable object can be saved in the StateManager.
		...
		{
			// Requests are not guaranteed to be processed in order nor at most once. The update function
			// here verifies that the incoming count is greater than the current count to preserve order.
